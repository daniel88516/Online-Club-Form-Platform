...
if ($action === 'join') {
    ...
    if ($club['is_public']) {
        ...
        $content = "【社團申請通知】\n用戶「{$_SESSION['username']}」申請加入您的社團「{$club['name']}」，請前往社團頁面審核。";
...
if ($action === 'invite') {
    ...
    $sys_id  = getSystemUserId($conn);
    $content = "【社團邀請】\n您被邀請加入社團「{$owner['name']}」，請前往社團頁面確認邀請。";
    $n = mysqli_prepare($conn, "INSERT INTO messages (sender_id, receiver_id, content) VALUES (?, ?, ?)");
    mysqli_stmt_bind_param($n, 'iis', $sys_id, $target_uid, $content);
    mysqli_stmt_execute($n);
    echo json_encode(['success' => true]);
...
if ($action === 'search_users') {
    ...
    $stmt = mysqli_prepare($conn, "
        ...
        WHERE u.username LIKE ? AND u.id != ?
          AND u.id NOT IN (SELECT user_id FROM club_members WHERE club_id = ? AND status IN ('active','invited'))
        LIMIT 10
    ");
    mysqli_stmt_bind_param($stmt, 'sii', $like, $user_id, $club_id);
    mysqli_stmt_execute($stmt);
    $res   = mysqli_stmt_get_result($stmt);
    $users = [];
    while ($row = mysqli_fetch_assoc($res)) {
        $users[] = ['id' => $row['id'], 'username' => $row['username'], 'avatar' => $row['avatar']];
    }
...
if ($action === 'owner_leave') {
    ...
    if ($next) {
        ...
        $sys_id  = getSystemUserId($conn);
        $content = "【社團管理員變更】\n社團「{$owner['name']}」的原管理員已離開，你已自動成為新管理員。";
        $n = mysqli_prepare($conn, "INSERT INTO messages (sender_id, receiver_id, content) VALUES (?, ?, ?)");
        mysqli_stmt_bind_param($n, 'iis', $sys_id, $new_owner_id, $content);
        mysqli_stmt_execute($n);
    } else {
